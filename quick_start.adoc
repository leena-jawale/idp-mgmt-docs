...
[IMPORTANT]
====
Before detaching cluster from hub, copy original OAuth from the ConfigMap named `idp-oauth-original` in the managed cluster's namespace on the hub cluster. After cluster is completely detached from hub cluster, manually edit the OAuth on the cluster which was managed by hub and update only spec from copied ConfigMap to OAuth. You should restore original OAuth on the managedcluster.
====
